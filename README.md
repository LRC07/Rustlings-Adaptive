...
生成一道经过**本地三重校验**的 5–50 行填空题 → 你在编辑器里补全 →
**解答评审门**（静态检查 + LLM 逻辑评审 + 存疑自动加测）→ **交互式复盘**
（原理解释校核、更优解挑战、双维度对比表）→ 归档进错误画像与错题本，
定向巩固。

核心理念是"小片段主义"：把复杂场景规约到一个个 5–50 行的可控片段
（行数按题面非空行计，注释计入；测试与参考解另计），质量容易把握，
便于针对性指导与统计。
...
- **交互式复盘**：解释校核（选择题/自由输入）、更优解挑战（只给方向）、
  机器实测 + LLM 四维对比表、下一步练习决策；评审/校核/对比三路
  并行调用模型，等待时间取最长一路而非逐段累加。
- **检查一下**：在对话里说"检查一下"，教练会读取练习文件并本地
  `rustc` 编译 + 测试取证（无需手动贴代码）。
- **学习画像**：错误码计数 + 概念 SM-2 间隔复习 + 错题本；同概念反复
  失败会收到主动提示，教练可随时查询画像定向出题。
- **用量与预算**：每次调用按用途（对话/出题·模板/出题·自由/评审/复盘）
  分项记账，实时展示花费，预算到顶自动拦截。
- **多模型 + 分场景路由**：OpenAI 兼容端点皆可（OpenAI / DeepSeek /
  Kimi / 本地 Ollama / vLLM…），`/model` 多档案切换（`new` 交互新建、
  `rm` 确认后删除），思考模式与推理强度按档案可配；`[routing]` 可把
  对话 / 出题·模板 / 出题·自由 / 复盘评审分别路由到不同模型（对话用
  快的、复盘用强的，账本按各环节实际模型分项记账）。
...
出题要响应快、成本低；自由生成与复盘评审要能力强。在 `config.toml`
的 `[routing]` 表里按环节指定档案（省略的字段沿用上一档/`active`）：

```toml
[routing]
chat = "Qwen3.8-Flash"      # 对话 / 工具环 / 假设实验室
generate = "Qwen3.8-Flash"  # 出题·模板路径（选题/填槽/改编）
generate_free = "GLM-5.3-Flash-High"  # 出题·自由生成（省略 = 沿用上一档）
...
| 出题·模板路径 | 响应快、成本低：选题/填槽是小型结构化调用，实测任何便宜模型都能做对 | Qwen3.8-Flash |
| 出题·自由生成 | 能力强：无模板兜底的长输出创作，质量与截断/超时都集中在这层（低频，贵一点值得）| GLM-5.3-Flash（推理 high，多轮实测选型胜出）|
