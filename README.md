...
你提问或贴报错 → 教练先用本地 `rustc` 编译你的代码，按编译结果讲解
...
**分功能配置模型（推荐）**：在 `[routing]` 表里按环节指定档案
（省略的字段沿用上一档/`active`）——对话与模板出题用快的，
自由生成与评审用强的：
...
各环节的模型要求（示例配置已内置推荐档案，可自行替换）：

| 环节 | 模型要求 | 示例推荐 |
|---|---|---|
| 对话 | 快、便宜（高频环节） | Qwen3.8-Flash |
| 模板出题 | 快、便宜——小型结构化调用，便宜模型就够 | Qwen3.8-Flash |
| 自由生成 | 要强——长输出创作，质量瓶颈在这层；低频，可以贵一点 | GLM-5.3-Flash（推理 high） |
| 评审、复盘 | 要强、可以慢 | Kimi-K3（开启思考） |

想要讲解更厚的对话风格，可以把 chat 换成 MiniMax-M3。
...
src/hypothesis_lab/ 假设实验（改前改后两版各编译一遍，对比报错差异）
...
taxonomy/           概念图谱定义
exercises/          练习目录（你生成的题落在 generated/，自动接线进 IDE）
```
