...
> M4.7 修复环预算与截断反馈、M4.8 hints 的实际效果待用真实端点验证（分层出题端到端冒烟）。
...
3. **对话出题并开练（M3/M4/M4.5a）**：输入"出一道 E0382 相关的练习"→
   ...
   通过后自动标记；卡住时按 `a` 问教练（代码+状态带回对话，教练本地
   编译取证后解释），`b` 返回对话；
...
1. **读本文件的状态表**，确定下一个里程碑（当前：M5 评审门+复盘；其后 M6）。
...
| `docs/复盘_M4.5-M4.8.md` | **最新交接**：出题域成果、★超时问题证据链与候选方案、M5 入口 |
